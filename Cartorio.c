...
#include <string.h> //bilbioteca responsavel por cuidar das string

int registro() //funcao responsavel por cadastrar os usuarios no sistema
{
	char arquivo[40]; //inicio da criacao de variaveis/string
	char cpf[40];
	char nome[40];
	char sobrenome[40];
	char cargo[40]; // final da criacao de variaveis/string
	
	printf("Digite o CPF a ser cadastrado:"); //coletando informacao do usuario
	scanf("%s", cpf); //%s refere-se a string
	
	strcpy(arquivo, cpf); //responssavel por cppiar os valores da string
	
	FILE *file;           // cria o arquivo eo "w" significa escrever
